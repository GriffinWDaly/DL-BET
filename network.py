...
class Recurrent_block_3D(nn.Module):
  def __init__(self, ch_out,t=2):
    super(Recurrent_block_3D,self).__init__()
    self.t = t
    self.ch_out = ch_out
    self.conv = nn.Sequential(
        nn.Conv3d(ch_out,ch_out,kernel_size = 3, stride = 1, padding = 1, bias = True),
...
class RRCNN_block_3D(nn.Module):
    def __init__(self, ch_in, ch_out, t=2):
        super(RRCNN_block_3D,self).__init__()
...
class R2AttU_Net3D(nn.Module):
  def __init__(self,img_ch=2, output_ch=1, t=2):
    super(R2AttU_Net,self).__init__()

    self.Maxpool = nn.MaxPool3d(kernel_stride=2,stride=2)
    self.Upsample = nn.Upsample(scale_factor=2)
    ...
    self.RRCNN5 = RRCNN_block_3D(ch_in=512,ch_out=1024,t=t)

    self.Up5 = up_conv_3D(ch_in=1024,ch_out=512)
    self.Att5 = Attention_block_3D(F_g=512,F_l=512,F_int=256)
    self.Up_RRCNN5 = RRCNN_block_3D(ch_in=1024, ch_out=512,t=t)
    
    self.Up4 = up_conv_3D(ch_in=512,ch_out=256)
    self.Att4 = Attention_block_3D(F_g=256,F_l=256,F_int=128)
    self.Up_RRCNN4 = RRCNN_block_3D(ch_in=512, ch_out=256,t=t)
    
    self.Up3 = up_conv_3D(ch_in=256,ch_out=128)
    self.Att3 = Attention_block_3D(F_g=128,F_l=128,F_int=64)
    self.Up_RRCNN3 = RRCNN_block_3D(ch_in=256, ch_out=128,t=t)
    
    self.Up2 = up_conv_3D(ch_in=128,ch_out=64)
    self.Att2 = Attention_block_3D(F_g=64,F_l=64,F_int=32)
    self.Up_RRCNN2 = RRCNN_block_3D(ch_in=128, ch_out=64,t=t)
